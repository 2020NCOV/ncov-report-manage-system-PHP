        ...
        if (!empty($_FILES)){
            ...
            if (!empty($file)){
                ...
                if($info){
                    ...
                    $msg = $msg."本次成功导入数据：".$import_success_count."条记录成功。其中(新增：".$import_success_add_count."，更新".$import_success_update_count.")"; 
                    if($import_success_count <$total_count ){
                    	$msg = "本次成功导入数据：".$import_success_count."条记录成功。其中(新增：".$import_success_add_count."，更新".$import_success_update_count.")<br> 最后导入成功记录序号为:".$last_num."(".$last_name.")<br>请检查数据，并修改正确后,重新导入"; 
                    }
                }else{
                    // 上传失败获取错误信息
                    echo $file->getError();
